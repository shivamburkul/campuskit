      ...
      <section>
        ...
        <div className="mt-3 space-y-3">
          {charges.map((charge, i) => (
            <div key={charge.id} className="grid grid-cols-1 gap-3 sm:grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] sm:gap-x-4 items-start">
              <TextField label={i === 0 ? 'Label' : ''} value={charge.label} onChange={(e) => updateCharge(charge.id, { label: e.target.value })} id={`charge-label-${charge.id}`} />
              <NumberField
              ...
              />
              <label className="flex items-center gap-1.5 pb-2.5 text-sm text-ink-700">
                <input type="checkbox" checked={!!charge.isDiscount} onChange={(e) => updateCharge(charge.id, { isDiscount: e.target.checked })} />
                Discount
              </label>
              <div className={i === 0 ? 'flex items-end pt-1 pl-4' : 'flex items-center pt-1 pl-4'}>
                <Button variant="ghost" type="button" onClick={() => setCharges((prev) => prev.filter((c) => c.id !== charge.id))} className="px-3">
                  Remove
                </Button>
              </div>
            </div>
