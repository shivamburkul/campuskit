...
export function JwtDecoder() {
  ...
  return (
    <div>
      <textarea value={token} onChange={(e) => setToken(e.target.value)} rows={4} placeholder="Paste a JWT (header.payload.signature)…" className="w-full resize-y rounded-md border border-ink-100 bg-surface p-3 font-mono text-sm shadow-sm outline-none focus:border-moss-500 focus:ring-1 focus:ring-moss-500" />
      <InlineNote>This decodes the token locally in your browser and does not verify the signature. Never paste a JWT you don&apos;t trust into a public website — even one that processes it locally.</InlineNote>
